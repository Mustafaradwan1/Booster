...
import { WinBoostingSection } from '@/type/boosting';
import Image, { StaticImageData } from 'next/image';
import React, { useEffect, useState } from 'react'

const WinBoosting = ({data} : {data:WinBoostingSection}) => {
  const [ImageActive, setImageActive] = useState<string | StaticImageData | null>(null);
  const boostingBoxes = data?.WinBoostingBox || [];
  ...
  if (!data || data.WinBoostingBox.imgs?.length === 0) return null;
  return (
        ...
        <div className='grid grid-cols-12 gap-5 mb-10'>
            <div className="col-span-12 bg-[#242836] rounded-2xl py-12 px-8">
              ...
              <div className="item mt-5 grid grid-cols-12 gap-[10px]">
                {boostingBoxes.rank?.map((item, ind) => (
                  <div
                    key={ind}
