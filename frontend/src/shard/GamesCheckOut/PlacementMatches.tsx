...
import { PlacementMatchesSection } from '@/type/boosting';
import Image, { StaticImageData } from 'next/image';
import React, { useEffect, useState } from 'react'

const PlacementMatches = ({data} : {data:PlacementMatchesSection}) => {
  const [ImageActive, setImageActive] = useState<string | StaticImageData | null>(null);
  const boostingBoxes = data?.PlacementMatchesBox || [];
  ...
  if (!data || data.PlacementMatchesBox.imgs?.length === 0) return null;
  return (
